  ...
  return (
    <>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-[10px] bg-white p-6 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <div>
          <h4 className="text-xl font-bold text-dark dark:text-white">Análisis de Récords Térmicos</h4>
          <p className="text-body text-sm font-medium">Filtra y visualiza el comportamiento extremo del mes seleccionado</p>
        </div>
        <div className="flex items-center gap-3">
          <select value={mesSeleccionado} onChange={(e) => setMesSeleccionado(e.target.value)} className="rounded border border-stroke bg-transparent px-4 py-2 font-medium text-dark outline-none dark:border-dark-3 dark:text-white">
            {Object.entries(mesesNombres).map(([valor, nombre]) => (
              <option key={valor} value={valor} className="text-dark dark:text-white bg-white dark:bg-gray-dark">{nombre}</option>
            ))}
          </select>
          <select value={añoSeleccionado} onChange={(e) => setAñoSeleccionado(Number(e.target.value))} className="rounded border border-stroke bg-transparent px-4 py-2 font-medium text-dark outline-none dark:border-dark-3 dark:text-white">
            {[2021, 2022, 2023, 2024, 2025].map((y) => (
              <option key={y} value={y} className="text-dark dark:text-white bg-white dark:bg-gray-dark">{y}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      {loading ? (
        <div className="mb-6 flex h-44 items-center justify-center rounded-[10px] border border-stroke bg-white dark:border-dark-3 dark:bg-gray-dark">
          ...
          <span className="text-body ml-3 text-base font-medium">Calculando mapas de calor y récords de la NASA...</span>
        </div>
        ...
        <div className="text-body mb-6 rounded-[10px] border border-stroke bg-white p-6 text-center dark:border-dark-3 dark:bg-gray-dark">
          No existen mediciones disponibles para el período de tiempo seleccionado.
        </div>
      ) : (
        <div className="mb-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
          ...
          <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark xl:col-span-2">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-base font-bold text-dark dark:text-white">Mapa de Calor: Máximas Diarias ({mesesNombres[mesSeleccionado]})</h3>
              <span className="text-body text-xs font-medium">Valores en °C</span>
            </div>
            <div className="grid grid-cols-5 gap-2 sm:grid-cols-7 md:grid-cols-8 lg:grid-cols-10">
              {datosMaximos.map((item, idx) => {
                ...
                return (
                  <div key={idx} className={`flex flex-col items-center justify-center rounded-lg p-2 shadow-sm transition-all hover:scale-105 ${obtenerColorCalor(item.y)}`} title={`Día ${item.x}: ${item.y}°C`}>
                    <span className="text-[10px] font-semibold opacity-70">Día {parseInt(numeroDia)}</span>
                    <span className="text-sm font-bold">{item.y.toFixed(1)}°</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="text-body mt-6 flex flex-wrap items-center gap-4 border-t border-stroke pt-4 text-xs font-medium dark:border-dark-3">
              ...
              <div className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-blue-100 dark:bg-blue-950"></span> ≤12° (Frío)</div>
              <div className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-green-100 dark:bg-green-950"></span> ≤16° (Templado)</div>
              <div className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-yellow-100 dark:bg-yellow-900/30"></span> ≤20° (Cálido)</div>
              <div className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-orange-200 dark:bg-orange-900/50"></span> ≤24° (Caluroso)</div>
              <div className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-orange-400 dark:bg-orange-700"></span> ≤28° (Muy Cálido)</div>
              <div className="flex items-center gap-1"><span className="h-3 w-3 animate-pulse rounded bg-red-500 dark:bg-red-600"></span> &gt;28° (Extremo)</div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-1 items-center justify-between rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
              <div>
                <span className="text-body text-xs font-bold uppercase tracking-wider"> Máximo Absoluto</span>
                <div className="mt-2 flex items-baseline gap-1">
                  <span className="text-4xl font-extrabold text-red-500 dark:text-red-400">{records.maxAbsoluta.toFixed(1)}</span>
                  <span className="text-body text-lg font-semibold">°C</span>
                </div>
                <p className="text-body mt-2 text-xs">Pico alcanzado el día: <span className="font-bold text-dark dark:text-white">{records.diaMax}</span></p>
              </div>
            </div>
            <div className="flex flex-1 items-center justify-between rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
              <div>
                <span className="text-body text-xs font-bold uppercase tracking-wider">Mínimo Absoluto</span>
                <div className="mt-2 flex items-baseline gap-1">
                  <span className="text-4xl font-extrabold text-blue-500 dark:text-blue-400">{records.minAbsoluta.toFixed(1)}</span>
                  <span className="text-body text-lg font-semibold">°C</span>
                </div>
                <p className="text-body mt-2 text-xs">Pico alcanzado el día: <span className="font-bold text-dark dark:text-white">{records.diaMin}</span></p>
              </div>
            </div>
          </div>
        </div>
